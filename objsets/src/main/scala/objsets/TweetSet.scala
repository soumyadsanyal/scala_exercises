    ...
    def union(that: TweetSet): TweetSet = {
      that match {
        ...
        case NonEmpty(elem, left, right) => {
          var temp1 = this.union(left)
          var temp2 = temp1.union(right)
          temp2.incl(elem)
        }
      }
    }
...
case class Empty() extends TweetSet {
    override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = this
...
case class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
    ...
    override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      if (p(elem)) {
        ...
        {
        acc.union(left.filterAcc(p, acc)).union(right.filterAcc(p, acc))
        }
    }
...
//      println(bestset.toString)
    ...
    override def descendingByRetweet: TweetList = {
                ...
                new Cons(best,rest.descendingByRetweet)
            }
